...
function get_products($catid) {
   ...
   $conn = db_connect();
   $query = "select * from product where catid = '".$conn->real_escape_string($catid)."'";
   $result = @$conn->query($query);
   if (!$result) {
     return false;
   }
...
function get_product_details($productid) {
  ...
  $conn = db_connect();
  $query = "select * from product where productid='".$conn->real_escape_string($productid)."'";
  $result = @$conn->query($query);
  if (!$result) {
     return false;
  }
...
function calculate_price($cart) {
  ...
  if(is_array($cart)) {
    ...
    foreach($cart as $productid => $qty) {
      $query = "select price from product where productid='".$conn->real_escape_string($productid)."'";
      $result = $conn->query($query);
      if ($result) {
